...
export function removeTransitionById(id) {
	...
	const transitionIds = (store.get(transition_list) ?? [])
		.map((transition, index) =>
			transition && getTransitionGroupId(transition) === targetGroupId
				? index
				: -1,
		)
...
export function changeTransitionBitLengths(inputDelta = 0, outputDelta = 0) {
	const { maxInput, maxOutput } = getEditorBitLengths();
	return setTransitionBitLengths(
		maxInput + inputDelta,
		maxOutput + outputDelta,
	);
}
...
export function handleInvalidTransitionFallback(inputValue, outputValue) {
	...
	const normalizedOutputValue = String(outputValue ?? "")
		.replace(/-/g, "x")
		.trim();
	const input = isValidBits(normalizedInputValue)
		? normalizedInputValue
		: "x".repeat(maxInput);
	const output = isValidBits(normalizedOutputValue)
	...
	store.set(
		alert,
		"Transition invalid: stored as unresolved (next state = x...x).",
	);
...
export function handleTransitionSave(labels) {
	...
	const duplicateExists = allTransitions.some((transition, index) => {
		...
		if (handleHiddenDontCareTransitions && transition.hiddenDontCare)
			return false;
	...
	if (
		...
		overlappingHiddenIds.forEach((hid) => {
			...
			const nextTo = Number.isFinite(activeTransition.to)
				? activeTransition.to
				: updated[hid].to;
