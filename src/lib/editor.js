...
function getNodeBitCount(nodes) {
	...
	return nodeCount <= 1
		? 1
		: Math.max(1, Math.ceil(Math.log2(Math.max(1, nodeCount))));
}
...
function expandTransitionForEditor(transition, nodes, nodeBitCount) {
	...
	return concreteTargets
		.map((binaryTarget) =>
			resolveNodeIdByBinary(nodes, binaryTarget, nodeBitCount),
		)
...
export function HandleStateClick(e, id) {
	...
	if (
		!["Add", "Remove", "Connect", "Label"].includes(store.get(editor_state))
	) {
		store.set(current_selected, (_prev) => id);
	}
}
...
export function handleShortCuts(key) {
	...
	if (
		["Controls", "Guide", "Save FSM", "settings"].includes(currentEditorState)
	) {
		return;
	}
...
export function getTransitionPoints(
	id1,
	id2,
	tr_id,
	nodesMap = null,
	...
	const allTransitions = (
		transitionsOverride || store.get(transition_list)
