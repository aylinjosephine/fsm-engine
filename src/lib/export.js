...
function normalizePatternBits(value, length, fill = "x", align = "left") {
	...
	return align === "left"
		? source.padStart(length, fill)
		: source.padEnd(length, fill);
}
...
function mergeBitPatterns(patterns, fallbackLength) {
	...
	return Array.from({ length }, (_, index) => {
		...
		return paddedPatterns.every((pattern) => pattern.charAt(index) === bit)
			? bit
			: "x";
	}).join("");
}
...
function getTransitionTargetPattern(transition, nodeBitCount, definedNodes) {
	...
	if (Number.isFinite(transition?.to) && transition.to >= 0) {
		const nodeBinary = Number(transition.to)
			.toString(2)
			.padStart(nodeBitCount, "0");
		const resolved = resolveNodeIdByBinary(
			definedNodes,
			nodeBinary,
			nodeBitCount,
		);
...
function collapseTransitionsForExport(transitions, definedNodes) {
	...
	const maxNodeId = definedNodes.reduce(
		(m, n) => Math.max(m, Number(n?.id ?? -1)),
		0,
	);
	const totalStates = Math.max(1, maxNodeId + 1);
	const nodeBitCount =
		totalStates <= 1 ? 1 : Math.max(1, Math.ceil(Math.log2(totalStates)));
	...
	return Array.from(groups.values()).map((group) => {
		...
		const mergedToBinaryId = mergeBitPatterns(
			group.map((transition) =>
				getTransitionTargetPattern(transition, nodeBitCount, definedNodes),
			),
			nodeBitCount,
		);
		...
		const output = resolveMooreTransitionOutput(
...
function resolveMooreTransitionOutput(transition, definedNodes) {
	...
	return String(transition?.output ?? transition?.mealy_output ?? "").replace(
		/-/g,
		"x",
	);
}
	...
	transitions.forEach((transition) => {
		...
		if (nodes[transition.from]) {
			nodes[transition.from] = {
				...nodes[transition.from],
				transitions: [
					...(nodes[transition.from].transitions || []),
					transitionRef,
				],
			};
		}
		...
		if (transition.from !== transition.to && nodes[transition.to]) {
			nodes[transition.to] = {
				...nodes[transition.to],
				transitions: [
					...(nodes[transition.to].transitions || []),
					transitionRef,
				],
			};
		}
...
function buildTransitionAtoms(transitions, existingTransitions, nodesMap) {
	...
	transitions.forEach((t) => {
		let existing =
			existingTransitions[t.id] ??
			existingTransitions.find((tr) => tr && tr.id === t.id);
		if (!existing && t.groupId != null) {
			existing = existingTransitions.find(
				(tr) => tr && (tr.groupId ?? tr.id) === t.groupId,
			);
		}
		const output = t.output ?? t.mealy_output ?? "";
		const groupId = t.groupId ?? existing?.groupId ?? t.id;

		let labelFromParent = String(t.label ?? existing?.label ?? "0/0").replace(
			/-/g,
			"x",
		);
...
function removeRenderedStates(existingNodeIds, newNodeList) {
	...
	const removedIds = existingNodeIds.filter(
		(id) => id != null && !newIds.has(id),
	);
...
function syncRenderedTransitions(transitionAtoms) {
	...
	transitionAtoms.forEach((transition) => {
		...
		if (textShape) {
			const formatted = String(labelText ?? "").replace(/x/g, "-");
			textShape.text(formatted);
		}
	});
...
function normalizeTransitionForParent(transition) {
	...
	const hasExplicitOutput =
		typeof transition?.output === "string" ||
		typeof transition?.mealy_output === "string";
	const hasExplicittoBinaryId = typeof transition?.toBinaryId === "string";
	const hasExplicitParentFields =
		hasExplicitInput || hasExplicitOutput || hasExplicittoBinaryId;

	const input = String(
		hasExplicitInput ? transition?.input : (inputFromLabel ?? ""),
	).replace(/-/g, "x");
	const isMoore = store.get(fsm_type) === "moore";
	const output = String(
		isMoore
	...
	const maxNodeId = allNodes.reduce(
		(m, n) => Math.max(m, Number(n?.id ?? -1)),
		0,
	);
	const totalStates = Math.max(1, maxNodeId + 1);
	...
	const shouldBeUnresolved =
		!!transition?.forceUnresolved ||
		(!hasExplicitParentFields && !labelIsValid);
	...
	const resolvedTargetId =
		typeof normalizedToBinaryId === "string" &&
		/^[01]+$/.test(normalizedToBinaryId)
			? resolveNodeIdByBinary(allNodes, normalizedToBinaryId, stateBits)
			: -1;
	const resolvedTo =
		resolvedTargetId >= 0 ? resolvedTargetId : (transition?.to ?? -1);
	const resolvedToBinaryId =
		resolvedTargetId >= 0 ? undefined : normalizedToBinaryId;
...
export function extractFsmData() {
	...
	const transitions = (store.get(transition_list) ?? []).filter(
		(transition) =>
			transition && !transition.isDraft && !transition.hiddenDontCare,
	);
	const fsmType = store.get(fsm_type) ?? "mealy";
	const visibleTransitions = collapseTransitionsForExport(
		transitions,
		definedNodes,
	);
	const visibleTransitionIds = new Set(visibleTransitions.map((t) => t.id));
	const visibleTransitionKeys = new Set(
		visibleTransitions.map((t) => `${t.from}:${t.input}`),
	);
	const preservedUnresolvedTransitions = unresolvedTransitions
		...
		.filter((t) => {
			...
			if (typeof t.toBinaryId === "string" && t.toBinaryId.length > 0)
				return true;
	...
	const exportedPreservedTransitions =
		fsmType === "moore" ? [] : preservedUnresolvedTransitions;
...
window.addEventListener("message", (event) => {
	...
	const nodeAtoms = [];
	const maxIncomingId = states.reduce(
		(m, s) => Math.max(m, Number(s?.id ?? -1)),
		0,
	);
	const nodeBitCount =
		maxIncomingId <= 0
			? 1
			: Math.max(1, Math.ceil(Math.log2(maxIncomingId + 1)));
	...
	if (false && isMoore) {
		transitions.forEach((transition) => {
			...
			const targetPattern = normalizePatternBits(
				transition.toBinaryId ??
					(transition.to >= 0 ? Number(transition.to).toString(2) : ""),
				nodeBitCount,
				"x",
				"left",
			);
			...
			if (concreteTargets.length > 0) {
				...
				if (fromExists) {
					...
					concreteTargets.forEach((binaryTarget) => {
						const resolvedTo = resolveNodeIdByBinary(
							nodeAtoms,
							binaryTarget,
							nodeBitCount,
						);
		...
		const mergedTransitions = (() => {
			...
			transitions.forEach((transition) => {
				const baseLabelInput = String(transition.input ?? "").replace(
					/-/g,
					"x",
				);
				const baseLabelOutput = isMoore
					? ""
					: String(transition.output ?? transition.mealy_output ?? "").replace(
							/-/g,
							"x",
						);
				const targetPattern = normalizePatternBits(
			...
			return Array.from(grouped.values()).map((entry) => {
				const fallbackLength = Math.max(
					1,
					...entry.inputs.map((input) => input.length),
				);
	...
	try {
		...
		const incomingIds = new Set(
			transitionAtoms.map((t) => t?.id).filter((id) => id != null),
		);
