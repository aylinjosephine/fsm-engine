  ...
  const handleCellChange = (fromId, alphabet, newToId) => {
    ...
    if (existingTr) {
      ...
      if (newName.length === 0) {
        ...
        newNodeList[existingTr.from].transitions = newNodeList[existingTr.from].transitions.filter(
          (t) => t.tr_name !== existingTrIndex,
        )
        if (existingTr.from !== existingTr.to) {
          newNodeList[existingTr.to].transitions = newNodeList[existingTr.to].transitions.filter(
            (t) => t.tr_name !== existingTrIndex,
          )
        }
                      ...
                      return EngineMode.alphabets.map((alpha, alphaIdx) => {
                        ...
                        return (
                          <td key={alphaIdx} className="p-2 border-r border-[#333]/50">
                            ...
                            >
                              <option value="trap" className="bg-[#252526] text-gray-500">
                                ø
                              </option>
                              {NodeList.map((n, i) => (
