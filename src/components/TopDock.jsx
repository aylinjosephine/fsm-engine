...
const TopDock = () => {
  ...
  const [hideDontCareTransitions, setHideDontCareTransitions] = useAtom(hide_dont_care_transitions)
  ...
  const dockItems = [
    ...
    {
      name: hideDontCareTransitions ? 'Hide x / -' : 'Show x / -',
      icon: <EyeOff stroke={iconFillColor} size={iconSize} />,
      condition: true,
      onclick: () => {
        setHideDontCareTransitions((value) => !value)
        setIsVisible(false)
      },
      active: hideDontCareTransitions,
    },
    {
          ...
          (item, idx) =>
            item.condition && (
              <button
                ...
                className={`flex gap-2 justify-center items-center font-github whitespace-nowrap text-base text-text-primary px-4 py-2 border border-border-bg rounded-lg cursor-pointer hover:-translate-y-1 hover:scale-102 active:scale-90 transition-all ease-in-out ${
                  item.active ? 'bg-secondary-bg' : 'bg-primary-bg/80'
                }`}
              >
                {item.icon}
                {item.name}
